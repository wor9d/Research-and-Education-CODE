...
for i in range(24) :
  ...
  for n in range(128):
      if i % 2 ==0:
        ...
        predicted_class = np.argmax(model.predict(x_test[n+128].reshape((1, 28, 28, 1))), axis=-1)
        true_class = np.argmax(y_test[n+128])  # 원-핫 인코딩을 정수로 변환

        if predicted_class[0] == true_class:
            static[n] = 1
        else:
          static2.append([n+128,np.argmax(y_test[n+128]), predicted_class[0]])


          plt.imshow(x_test[n+128].reshape(28, 28), cmap='gray')  # 이미지가 흑백일 경우 cmap='gray'
          plt.axis('off')
          plt.show()
